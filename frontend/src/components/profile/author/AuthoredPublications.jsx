...
import '../../../styles/UserPublications.css'; // используем те же стили
...
const AuthoredPublications = ({ authorId }) => {
    ...
    const fetchPublications = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:3000/authors/${authorId}/publications`);
            if (!res.ok) {
                const errData = await res.json().catch(() => ({}));
                if (res.status === 404 && errData.error === 'No publications found for this author') {
                    setPublications([]);
                    return;
                }
